<?php
    ...
    if(!empty($_SESSION["email"])) {
        ...
        if ( isset( $_POST['checkout'] ) ) {
            ...
            $name = protect($con, $_POST['op_name']);
            $email = protect($con, $_POST['op_mail']);
            $mobile = protect($con, $_POST['op_mobile']);
            $coupon_code = protect($con, $_POST['coupon_code']);
            $delivery_address = protect($con, $_POST['op_delivery']);
            $payment_type = protect($con, $_POST['payment_type']);
        ...
        if( isset ($_POST['proceed'] ) ) {
            $order = protect($con, $_POST['order']);
            $data = base64_decode($order);
            $name = protect($con, $_POST['firstname']);
            $email = protect($con, $_POST['email']);
            $order_number = protect($con, $_POST['productinfo']);
            $mobile = protect($con, $_POST['phone']);
            $total_price = protect($con, $_POST['amount']);
            $payment_method = protect($con, $_POST['payment_method']);
            $payment_type = protect($con, $_POST['payment_type']);
            $delivery_address = protect($con, $_POST['delivery_address']);
            //die($order . "<br><br>" . base64_decode($order));
            $sql = "INSERT INTO `orders` (`o_id`, `order_number`, `name`, `email`, `mobile`, `billing_address`, `delivery_address`, `payment_method`, `payment_type`, `status`, `order_data`, `price`, `order_date`, `delivery_date`, `session`) VALUES (NULL, '$order_number', '$name', '$email', '$mobile', 'NULL', '$delivery_address', '$payment_method', '$payment_type', 'NULL', '$data', '$total_price', CURDATE(), NULL, '$session_mail');";
        ...
        <section>
            <div class="container pad">
                <form action="<?php echo $action; ?>" method="POST" name="payuForm">
                    ...
                    <input type="hidden" name="amount" class="form-control" readonly value="<?php echo (empty($posted['amount'])) ? '' . $offer_price . '' : $posted['amount'] ?>" />
                    ...
                    <?php } ?>
                    <a href="cart.php"><button class="btn btn-danger uk-align-center btn-round btn-sm">Cancel Payment</button></a>
                </form>
            </div>
        </section>
