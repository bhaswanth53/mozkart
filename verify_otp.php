<?php
    ...
    if(!empty($_SESSION["email"])) {
        ...
        $html = "
            ...
            <p>Please use above code to retrieve your password</p>
        ";
        ...
        $mail->isHTML(true);
        $mail->Subject = 'New Request Recieved';
        $mail->Body = $html;
        if(!$mail->send()) {
            header('location: forgot_otp.php?success=false');
            $sql ="UPDATE user SET forgot = '1' WHERE email = '$session_mail'";
        }
        else {
            $update_status = mysqli_query($con,"UPDATE user SET forgot = '1' WHERE email = '$session_mail'");
    ...
    <body>
        ...
        <section class="">
            <div class="container pad">
                <div class="">
                <p class="font-size text-center text-primary">Please verify the one time password sent to your email address</p>
                    <div class="card card-nav-tabs text-center uk-width-1-2@m mx-auto">
                        <div class="card-header card-header-primary">
                            One Time Password
                        </div>
                        <div class="card-body">
                            <form action="otp_process.php" method="POST" id="verify_otp">
                                <div class="uk-margin uk-margin-top">
                                    <input type="hidden" name="sent_otp" value="<?php echo md5(base64_encode($otp)); ?>">
                                    <input type="password" class="uk-input" name="otp" placeholder="One Time Password">
                                </div>
