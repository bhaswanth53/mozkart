<?php
	...
	if ( !empty($_SESSION['admin'] ) ) {
		...
		if (isset($_GET['delete'])) {
			...
			$que = mysqli_query($con, "DELETE FROM products WHERE product_id = $rid");
			header('location: products.php');
			// $ques = mysqli_query($con, "SELECT * FROM `story` WHERE query_id = $rid") or die(mysqli_error($con));
		}
		else if (isset($_GET['status'])) {
			$pstatus = protect($con, $_GET['status']);
			$sel = mysqli_query($con, "SELECT * FROM `products` WHERE product_id = '$pstatus'");
			$avail = mysqli_fetch_assoc($sel);
			$pavail = $avail['availability_status'];
			if($pavail == 0) {
				$sql = "UPDATE `products` SET `availability_status` = '1' WHERE `products`.`product_id` = '$pstatus';";
				mysqli_query($con, $sql);
			}
			else {
				$sql = "UPDATE `products` SET `availability_status` = '0' WHERE `products`.`product_id` = '$pstatus';";
				mysqli_query($con, $sql);
			}
			header('location: products.php');
			// $ques = mysqli_query($con, "SELECT * FROM `story` WHERE query_id = $rid") or die(mysqli_error($con));
                ...
                <table class="table table-hover">
                    ...
                    <tbody id="myTable">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="edit_product.php?edit=<?php echo $product['product_id']; ?>" class="btn btn-primary btn-xs" title="Edit Product"><i class="fa fa-edit"></i></a>
                                <a href="products.php?delete=<?php echo $product['product_id']; ?>" class="btn btn-danger btn-xs" title="Delete Product"><i class="fa fa-trash"></i></a>
                                <?php 
                                    $status = $product['availability_status'];
                                    $id = $product['product_id'];
                                    if( $status == 0 ) {
                                        $button = "<a href='products.php?status=$id' class='btn btn-danger btn-xs' title='Status Not Available'><i class='fa fa-close'></i></a>";
                                    }
                                    else {
                                        $button = "<a href='products.php?status=$id' class='btn btn-success btn-xs' title='Status Available'><i class='fa fa-check'></i></a>";
                                    }
                                    echo $button;
                                ?>
                            </td>
